...
export function TableFilters({ tags }: { tags: { id: string; name: string }[] }) {
  ...
  const nuqsOptions = {
    shallow: false,
    startTransition,
    limitUrlUpdates: { method: 'debounce' as const, timeMs: 300 },
  };

  const [q, setQ] = useQueryState('q', parseAsString.withDefault('').withOptions(nuqsOptions));
  const [tag, setTag] = useQueryState('tag', parseAsString.withDefault('').withOptions(nuqsOptions));
  const [difficulty, setDifficulty] = useQueryState(
    'difficulty',
    parseAsString.withDefault('').withOptions(nuqsOptions),
  );
  const [status, setStatus] = useQueryState('status', parseAsString.withDefault('').withOptions(nuqsOptions));
  const [, setPage] = useQueryState('page', parseAsInteger.withDefault(1).withOptions(nuqsOptions));
  ...
  return (
    <div
      className={`flex flex-wrap items-end gap-3 rounded-md border border-border-default bg-bg-surface p-4 transition-opacity ${isPending ? 'opacity-60' : ''}`}
    >
